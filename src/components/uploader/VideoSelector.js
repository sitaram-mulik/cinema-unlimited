...
import { catPills } from '../../constants/video';
...
export default function VideoSelector({ uploadForm, updateUploadForm, isEdit }) {
  const { localVideoFile, category } = uploadForm;

  return (
    <CUScrollContainer>
      <CUHeading>Select Video</CUHeading>
      <View className="mb-4 flex-row ">
        ...
        <View className="mt-8">
          ...
          {!localVideoFile && uploadForm.id && (
            <BunnyPlayer videoUrl={`${buVideoStreamURL}/${uploadForm.id}`} />
          )}
          {/* <Slider
