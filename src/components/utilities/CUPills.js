...
export default function CUPills({ items, selectedItem, setSelectedItem, radio, title, className }) {
        ...
        {items.map(item => {
          return (
            <TouchableOpacity key={item} onPress={() => onPillPress(item)}>
              <CUText
                className={`text-md rounded-full border border-secondary px-2 py-1 mr-3 mb-3 ${selectedItem === item || (selectedItem?.find && selectedItem?.find(s => s === item)) ? 'bg-backgroundSecondary text-primary border-secondary' : ''}`}
              >
                {item}
              </CUText>
            </TouchableOpacity>
